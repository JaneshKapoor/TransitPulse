...
def get_snowflake_connection():
    conn = snowflake.connector.connect(
        user=st.secrets["SNOWFLAKE_USER"],  # Snowflake username
        password=st.secrets["SNOWFLAKE_PASSWORD"],  # Snowflake password
        account=st.secrets["SNOWFLAKE_ACCOUNT"],  # Account identifier
